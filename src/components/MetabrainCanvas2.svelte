<script>
	...
	// Object pool for Line instances
	const linePool = [];
	...
	function init() {
	  lines.forEach(releaseLine);
	  lines.length = 0;
  
	  for (let i = 0; i < data.initialLines; ++i) {
		setStartCoords();
		let line = getLine(starter);
	...
	Line.prototype.reset = function(parent) {
	  this.x = parent.x | 0;
	  this.y = parent.y | 0;
	...
	Line.prototype.step = function() {
	  ...
	  if (this.lineDist <= 0) {
		...
		if (lines.length < data.maxLines) lines.push(getLine(this));
		if (Math.random() < data.lineBirthFactor) {
		  lines.push(getLine(this));
		  let index = (Math.random() * lines.length) | 0;
		  releaseLine(lines[index]);
